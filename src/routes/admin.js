...
router.put('/activities/:id', async (req, res) => {
  const { title, type_slug, start_time, end_time, signup_deadline, location, organizer, quota, description, gradient } = req.body
  try {
    const fixTime = t => t ? t.replace('T', ' ').substring(0, 19) : null
    await db.query(
      `UPDATE activities SET title=?, type_slug=?, start_time=?, end_time=?, signup_deadline=?,
       location=?, organizer=?, quota=?, description=?, gradient=? WHERE id=?`,
      [title, type_slug, fixTime(start_time), fixTime(end_time), fixTime(signup_deadline), location, organizer, quota || 0, description, gradient, req.params.id]
    )
